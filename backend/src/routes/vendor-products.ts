  ...
  async (req, res) => {
    try {
      ...
      const peRow: Record<string, unknown> = {
        ...
        unit_purchased:
          vendorProduct.initial_unit_purchased ?? (caseUnit == null ? 1 : null),
      };
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Keep behavior consistent with POST /vendor-products:
      // when manual bulk recording creates/enables viable ingredient pricing,
      // run undeprecate re-evaluation for affected parent items.
...
/**
 * POST /vendor-products/bulk/invoice-import
 * Invoice import confirm: existing invoice price events + new VVP/mapping/events atomically.
 * Runs autoUndeprecate after success (mapping exists before undeprecate).
 */
router.post(
  "/bulk/invoice-import",
  unifiedAuthorizationMiddleware(
  ...
  async (req, res) => {
    try {
      ...
      const { data, error } = await supabase.rpc("record_invoice_import_atomic", {
        p_tenant_id: selectedTenantId,
        p_user_id: req.user!.id,
        p_operations: operations,
      });
