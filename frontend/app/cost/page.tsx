  ...
  const performSave = async (itemsToSave: PreppedItem[]) => {
    ...
    try {
      ...
      const filteredItems = currentItems.filter((item) => {
        ...
        const itemName = getItemDisplayName(item, baseItems);
        if (itemName.trim() === "" && item.proceed_yield_amount === 0) {
          return false;
        }
        return true;
      });
        ...
        if (item.isNew) {
          ...
          if (
            ...
            itemsToUpdate.push({
              id: item.id,
              data: {
                // Prepped Itemの場合はnameが必須（Raw Itemはこのページでは扱わない）
                name: item.name || "",
                is_menu_item: item.is_menu_item,
                proceed_yield_amount: item.proceed_yield_amount,
                proceed_yield_unit: item.proceed_yield_unit,
      ...
      if (newlyCreatedItemIds.length > 0) {
        try {
          ...
          console.error(
            "Failed to ensure standard technical sheet v0:",
            ensureError,
          );
        }
      }
                      ...
                      const expandedBgClass =
                        expandedColorIndex !== undefined
                          ? (() => {
                              ...
                              return (
                                addLikeColors[expandedColorIndex % 2] || ""
                              );
                            })()
                          : "";
                        ...
                        <Fragment key={item.id}>
                          {activeMode === "access-control" ? (
                              ...
                              >
                                {isTenantAdminOrDirector &&
                                  (() => {
                                    ...
                                    const pending =
                                      pendingCrossTenantChanges.get(item.id);
                                    const isHideChecked =
                                      pending !== undefined
                                        ? pending === "hide"
                            ...
                            <tr
                              ...
                              style={{
                                height: finishAmountEachEditRow
                                  ? "auto"
                                  : "51px",
                                minHeight: "51px",
