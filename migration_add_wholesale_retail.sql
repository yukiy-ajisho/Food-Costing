-- Wholesale/Retail機能追加のためのマイグレーション

-- Step 1: itemsテーブルにカラム追加
ALTER TABLE items
  ADD COLUMN IF NOT EXISTS wholesale numeric,
  ADD COLUMN IF NOT EXISTS retail numeric;

-- Step 2: コスト内訳を計算する関数（シンプル化・改良版）
CREATE OR REPLACE FUNCTION calculate_item_costs_with_breakdown()
RETURNS TABLE (
  out_item_id uuid,
  out_item_name text,
