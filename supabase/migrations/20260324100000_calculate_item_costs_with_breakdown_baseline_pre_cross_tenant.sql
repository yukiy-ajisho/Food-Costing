-- =============================================================================
-- Baseline snapshot: public.calculate_item_costs_with_breakdown(uuid)
-- =============================================================================
-- ソース: supabase/schema_dump.sql 上の定義と同一（作業時点 2026-03-24 確認）。
-- 目的:
--   1) 通常のマイグレーション適用で DB にこの定義を載せられる（現状と同じなら実質ほぼ no-op）。
--   2) cross-tenant 対応などで本関数を変更した後、このファイル全文を SQL Editor や
--      psql で再実行すれば、このベースラインへ戻せる（手動ロールバック用）。
-- 注意:
--   Supabase の履歴は線形のため「同じファイルを履歴に二重登録」はできない。
--   恒久ロールバックをマイグレーションで載せる場合は、この内容をコピーした
--   新しいタイムスタンプの migration として追加する。
-- =============================================================================
-- cross-tenant 版 (uuid, integer) が入っている環境から戻すときは先に DROP が必要
DROP FUNCTION IF EXISTS public.calculate_item_costs_with_breakdown(uuid, integer);

CREATE OR REPLACE FUNCTION public.calculate_item_costs_with_breakdown(p_tenant_id uuid)
RETURNS TABLE (
  out_item_id uuid,
  out_item_name text,
...
ALTER FUNCTION public.calculate_item_costs_with_breakdown(uuid) OWNER TO postgres;

GRANT ALL ON FUNCTION public.calculate_item_costs_with_breakdown(uuid) TO anon;
GRANT ALL ON FUNCTION public.calculate_item_costs_with_breakdown(uuid) TO authenticated;
GRANT ALL ON FUNCTION public.calculate_item_costs_with_breakdown(uuid) TO service_role;
