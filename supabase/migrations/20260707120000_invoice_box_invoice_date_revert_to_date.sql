-- invoice_box_invoices.invoice_date: timestamptz → date (drop time; keep calendar day in UTC).
...
ALTER TABLE public.invoice_box_invoices
  ...
  USING (
    CASE
      ...
      ELSE (invoice_date AT TIME ZONE 'UTC')::date
    END
  );

COMMENT ON COLUMN public.invoice_box_invoices.invoice_date IS
  'Invoice creation date (calendar date).';
