-- Fix: WL leaf MCL cost = wholesale $/kg only (no recipe labor added on return).
-- Re-apply if 20260621120000 was already applied without this fix.

-- Franchise MCL: scoped breakdown + wholesale list price leaf overrides (members-only map).
-- Does not modify calculate_item_costs_with_breakdown_scoped.

CREATE OR REPLACE FUNCTION public.calculate_item_costs_with_breakdown_wholesale_overrides(
  p_tenant_id uuid,
  p_call_depth integer,
  p_seed_item_ids uuid[],
  p_wholesale_list_id uuid
)
RETURNS TABLE (
  out_item_id uuid,
  out_item_name text,
...
COMMENT ON FUNCTION public.calculate_item_costs_with_breakdown_wholesale_overrides(uuid, integer, uuid[], uuid) IS
  'Franchise MCL: WL wholesale leaf = that $/kg as cost (no added labor); else scoped breakdown (food + labor).';

GRANT ALL ON FUNCTION public.calculate_item_costs_with_breakdown_wholesale_overrides(uuid, integer, uuid[], uuid) TO anon;
GRANT ALL ON FUNCTION public.calculate_item_costs_with_breakdown_wholesale_overrides(uuid, integer, uuid[], uuid) TO authenticated;
GRANT ALL ON FUNCTION public.calculate_item_costs_with_breakdown_wholesale_overrides(uuid, integer, uuid[], uuid) TO service_role;
