-- =============================================================================
-- Cross-tenant: 共有された他テナントの prepped を材料に含めた breakdown
-- =============================================================================
-- ・calculate_item_costs_with_breakdown(p_tenant_id, p_call_depth) に拡張。
--   p_call_depth は省略可（既定 0）。既存の supabase.rpc({ p_tenant_id }) はそのまま。
-- ・閲覧テナントの recipe_lines に foreign prepped があり、cross_tenant_item_shares で
--   read が効いている場合のみ、オーナーテナント文脈で同関数を再帰（temp リネームで衝突回避）。
-- ・再帰深さ上限 8。ベースライン復元は 20260324100000_* のファイルを参照。
-- =============================================================================

-- 旧シグネチャ (uuid) と並立すると PostgREST / 呼び出しが曖昧になるため削除
DROP FUNCTION IF EXISTS public.calculate_item_costs_with_breakdown(uuid);
...
BEGIN
  ...
  LOOP
    ALTER TABLE temp_item_costs RENAME TO _ct_brk_pause_tic;
    ALTER TABLE temp_labor_costs RENAME TO _ct_brk_pause_tlc;
    ALTER TABLE temp_ingredient_edges RENAME TO _ct_brk_pause_tie;
    ALTER TABLE temp_ingredient_counts RENAME TO _ct_brk_pause_ticnt;
    ALTER TABLE temp_ct_foreign_needed RENAME TO _ct_brk_pause_fn;

    INSERT INTO _ct_brk_pause_tic (item_id, item_name, food_cost_per_gram, labor_cost_per_gram)
    SELECT
      fr.out_item_id,
      fr.out_item_name,
      fr.out_food_cost_per_gram,
      fr.out_labor_cost_per_gram
    FROM calculate_item_costs_with_breakdown(r_owner.owner_tid, p_call_depth + 1) fr
    INNER JOIN _ct_brk_pause_fn n
      ON n.child_item_id = fr.out_item_id
     AND n.owner_tid = r_owner.owner_tid
    ON CONFLICT (item_id) DO NOTHING;

    ALTER TABLE _ct_brk_pause_tic RENAME TO temp_item_costs;
    ALTER TABLE _ct_brk_pause_tlc RENAME TO temp_labor_costs;
    ALTER TABLE _ct_brk_pause_tie RENAME TO temp_ingredient_edges;
    ALTER TABLE _ct_brk_pause_ticnt RENAME TO temp_ingredient_counts;
    ALTER TABLE _ct_brk_pause_fn RENAME TO temp_ct_foreign_needed;
  END LOOP;
...
-- 既存の 1 引数呼び出し互換: 同名で uuid のみのオーバーロードを残すと曖昧になるため、
-- 上記は (uuid, int default) の 1 シグネチャに統一。PostgREST は DEFAULT で単一引数 OK。
