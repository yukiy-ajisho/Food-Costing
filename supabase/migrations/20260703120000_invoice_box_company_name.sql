-- Snapshot company name on invoice_box_invoices for Invoice Box display/filter.

ALTER TABLE public.invoice_box_invoices
  ADD COLUMN IF NOT EXISTS company_name text NOT NULL DEFAULT '';

UPDATE public.invoice_box_invoices ibi
SET company_name = COALESCE(ia.company_name, '')
FROM public.delivery_sites ds
LEFT JOIN public.invoicing_accounts ia ON ia.id = ds.account_id
WHERE ibi.delivery_site_id = ds.id
  AND ibi.company_name = '';
