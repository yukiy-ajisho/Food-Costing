-- grandfather: Hide 後も閲覧テナントの既存 recipe_lines が他テナント prepped を参照している場合は
-- temp_ct_foreign_needed に含め、原価グラフで材料コストを織り込む。
...
COMMENT ON FUNCTION public.calculate_item_costs_with_breakdown_scoped(uuid, integer, uuid[]) IS
  'Phase2 scoped breakdown + grandfather: foreign prepped allowed when read share OR local recipe line references child (Hide 後の既存行).';
