-- =============================================================================
-- Fix: prevent cross-tenant recursive bounce in breakdown calculation
-- =============================================================================
-- This migration keeps the temp-table collision fix and adds a tenant-level
-- re-entry guard in the same DB session to stop A -> B -> A -> ... recursion.
-- =============================================================================
...
DECLARE
  ...
  v_pause_ticnt text;
  v_pause_fn text;
  v_guard_table_exists boolean;
  v_tenant_already_active boolean;
BEGIN
  ...
  -- Tenant recursion guard (session-scoped temp table).
  v_guard_table_exists := to_regclass('pg_temp.temp_ct_active_tenants') IS NOT NULL;
  IF NOT v_guard_table_exists THEN
    CREATE TEMP TABLE temp_ct_active_tenants (
  ...
  SELECT EXISTS (
    SELECT 1
    FROM temp_ct_active_tenants
    WHERE tenant_id = p_tenant_id
  ) INTO v_tenant_already_active;

  -- If this tenant is already on the active recursion stack, stop this branch.
  IF v_tenant_already_active THEN
    RETURN;
  END IF;

  INSERT INTO temp_ct_active_tenants (tenant_id)
  VALUES (p_tenant_id)
  ...
  EXCEPTION
    WHEN OTHERS THEN
      DELETE FROM temp_ct_active_tenants WHERE tenant_id = p_tenant_id;
      RAISE;
  END;
