...
CREATE OR REPLACE FUNCTION "public"."backfill_labor_into_standard_snapshot"("p_snapshot" "jsonb", "p_source_item_id" "uuid", "p_tenant_id" "uuid") RETURNS "jsonb"
...
ALTER FUNCTION "public"."backfill_labor_into_standard_snapshot"("p_snapshot" "jsonb", "p_source_item_id" "uuid", "p_tenant_id" "uuid") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."backfill_labor_into_standard_snapshot"("p_snapshot" "jsonb", "p_source_item_id" "uuid", "p_tenant_id" "uuid") IS 'One-time helper: merge labor_rows, total_labor_cost, cost_inputs.by_labor_role, and recipe_snapshot labor lines from live recipe_lines. Idempotent if labor_rows already non-empty.';
...
CREATE OR REPLACE FUNCTION "public"."calculate_item_costs_with_breakdown_wholesale_overrides"("p_tenant_id" "uuid", "p_call_depth" integer, "p_seed_item_ids" "uuid"[], "p_wholesale_list_id" "uuid") RETURNS TABLE("out_item_id" "uuid", "out_item_name" "text", "out_total_cost_per_gram" numeric, "out_food_cost_per_gram" numeric, "out_labor_cost_per_gram" numeric)
    LANGUAGE "plpgsql"
    AS $_$
DECLARE
  v_rows_inserted integer;
...
BEGIN
  ...
  BEGIN
    ...
    DROP TABLE IF EXISTS temp_ingredient_counts;
    DROP TABLE IF EXISTS temp_ct_foreign_needed;
    DROP TABLE IF EXISTS temp_wl_override;
    DROP TABLE IF EXISTS temp_needed_local_prepped;
    DROP TABLE IF EXISTS temp_wl_override;
    ...
    CREATE INDEX ON temp_labor_costs(item_id);
    -- WL override map: active members + latest numeric wholesale ($/kg). Leaf = that rate only.
    CREATE TEMP TABLE temp_wl_override (
      item_id uuid PRIMARY KEY,
      food_cost_per_gram numeric NOT NULL
    );

    INSERT INTO temp_wl_override (item_id, food_cost_per_gram)
    SELECT s.item_id, (s.wholesale_price / 1000.0)::numeric
    FROM (
      SELECT DISTINCT ON (wll.item_id)
        wll.item_id,
        wll.wholesale_price
      FROM wholesale_list_members wlm
      INNER JOIN wholesale_list_lines wll
        ON wll.wholesale_list_id = wlm.wholesale_list_id
       AND wll.item_id = wlm.item_id
      WHERE wlm.wholesale_list_id = p_wholesale_list_id
      ORDER BY wll.item_id, wll.created_at DESC
    ) s
    WHERE s.wholesale_price IS NOT NULL
      AND s.wholesale_price > 0;

    CREATE INDEX ON temp_wl_override(item_id);
    ...
    WITH RECURSIVE needed(item_id) AS (
      ...
      WHERE rl.line_type = 'ingredient'
        ...
        AND ci.tenant_id = p_tenant_id
        AND NOT EXISTS (
          SELECT 1 FROM temp_wl_override w WHERE w.item_id = rl.child_item_id
        )
    )
    INSERT INTO temp_needed_local_prepped(item_id)
    SELECT DISTINCT item_id FROM needed
    ...
    CREATE INDEX ON temp_ingredient_counts(parent_item_id);
    -- Pre-seed WL leaf items (do not expand their recipe_lines).
    INSERT INTO temp_item_costs (item_id, item_name, food_cost_per_gram, labor_cost_per_gram)
    SELECT i.id, i.name, w.food_cost_per_gram, 0::numeric
    FROM temp_wl_override w
    INNER JOIN items i ON i.id = w.item_id AND i.tenant_id = p_tenant_id
    WHERE w.item_id = ANY(v_seed_item_ids_norm)
       OR w.item_id IN (SELECT DISTINCT e.child_item_id FROM temp_ingredient_edges e)
    ON CONFLICT (item_id) DO NOTHING;
    ...
    LOOP
      ...
      EXECUTE format($q$
        ...
        SELECT
          ...
          fr.out_food_cost_per_gram,
          fr.out_labor_cost_per_gram
        FROM public.calculate_item_costs_with_breakdown_wholesale_overrides($1, $2, $3, $5) fr
        INNER JOIN %I n
          ON n.child_item_id = fr.out_item_id
         AND n.owner_tid = $4
        ON CONFLICT (item_id) DO NOTHING
      $q$, v_pause_tic, v_pause_fn)
      USING r_owner.owner_tid, p_call_depth + 1, v_owner_seeds, r_owner.owner_tid, p_wholesale_list_id;
    ...
    LOOP
      ...
      SELECT
        ...
        parent.name,
        SUM(
          COALESCE(wl_child.food_cost_per_gram, tc.food_cost_per_gram) *
          CASE
            WHEN rl.unit = 'kg' THEN rl.quantity * 1000
            WHEN rl.unit = 'lb' THEN rl.quantity * 453.592
            WHEN rl.unit = 'oz' THEN rl.quantity * 28.3495
        ...
        SUM(
          (CASE
            WHEN wl_child.item_id IS NOT NULL THEN 0::numeric
            ELSE tc.labor_cost_per_gram + COALESCE(
            tlc_child.labor_cost_batch / NULLIF(
              CASE
                WHEN child_items.proceed_yield_unit = 'kg' THEN child_items.proceed_yield_amount * 1000
                WHEN child_items.proceed_yield_unit = 'g' THEN child_items.proceed_yield_amount
      ...
      INNER JOIN temp_ingredient_edges rl ON parent.id = rl.parent_item_id
      INNER JOIN temp_item_costs tc ON rl.child_item_id = tc.item_id
      LEFT JOIN temp_wl_override wl_child ON wl_child.item_id = rl.child_item_id
      LEFT JOIN items child_items ON rl.child_item_id = child_items.id
      LEFT JOIN base_items bi ON child_items.base_item_id = bi.id
      LEFT JOIN temp_labor_costs tlc_child ON child_items.id = tlc_child.item_id
      WHERE parent.item_kind = 'prepped'
    ...
    FROM (
      SELECT
        ...
        (
          CASE
            WHEN wl.item_id IS NOT NULL THEN wl.food_cost_per_gram
            ELSE tc.food_cost_per_gram + COALESCE(tc.labor_cost_per_gram, 0) + COALESCE(
              lc.labor_cost_batch / NULLIF(
                CASE
                  WHEN i.proceed_yield_unit = 'kg' THEN i.proceed_yield_amount * 1000
                  WHEN i.proceed_yield_unit = 'g' THEN i.proceed_yield_amount
        ...
        (
          CASE
            WHEN wl.item_id IS NOT NULL THEN wl.food_cost_per_gram
            ELSE tc.food_cost_per_gram
          END
        )::numeric AS out_food_cost_per_gram,
        (
          CASE
            WHEN wl.item_id IS NOT NULL THEN 0::numeric
            ELSE COALESCE(tc.labor_cost_per_gram, 0) + COALESCE(
              lc.labor_cost_batch / NULLIF(
                CASE
                  WHEN i.proceed_yield_unit = 'kg' THEN i.proceed_yield_amount * 1000
                  WHEN i.proceed_yield_unit = 'g' THEN i.proceed_yield_amount
      ...
      FROM temp_item_costs tc
      LEFT JOIN temp_wl_override wl ON wl.item_id = tc.item_id
      LEFT JOIN items i ON tc.item_id = i.id
      LEFT JOIN temp_labor_costs lc ON tc.item_id = lc.item_id
      WHERE tc.food_cost_per_gram IS NOT NULL
        AND i.tenant_id = p_tenant_id
...
ALTER FUNCTION "public"."calculate_item_costs_with_breakdown_wholesale_overrides"("p_tenant_id" "uuid", "p_call_depth" integer, "p_seed_item_ids" "uuid"[], "p_wholesale_list_id" "uuid") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."calculate_item_costs_with_breakdown_wholesale_overrides"("p_tenant_id" "uuid", "p_call_depth" integer, "p_seed_item_ids" "uuid"[], "p_wholesale_list_id" "uuid") IS 'Franchise MCL: WL wholesale leaf = that $/kg as cost (no added labor); else scoped breakdown (food + labor).';
...
CREATE OR REPLACE FUNCTION "public"."ensure_standard_technical_sheet_v0_if_absent"("p_tenant_id" "uuid", "p_source_item_id" "uuid", "p_snapshot" "jsonb", "p_created_by" "uuid") RETURNS TABLE("id" "uuid", "version_number" integer, "is_latest" boolean, "created" boolean)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."ensure_standard_technical_sheet_v0_if_absent"("p_tenant_id" "uuid", "p_source_item_id" "uuid", "p_snapshot" "jsonb", "p_created_by" "uuid") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."insert_standard_technical_sheet_version_atomic"("p_tenant_id" "uuid", "p_source_item_id" "uuid", "p_snapshot" "jsonb", "p_created_by" "uuid", "p_description" "text" DEFAULT NULL::"text", "p_procedure" "text" DEFAULT NULL::"text") RETURNS TABLE("id" "uuid", "version_number" integer, "is_latest" boolean)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."insert_standard_technical_sheet_version_atomic"("p_tenant_id" "uuid", "p_source_item_id" "uuid", "p_snapshot" "jsonb", "p_created_by" "uuid", "p_description" "text", "p_procedure" "text") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."insert_standard_technical_sheet_version_atomic"("p_tenant_id" "uuid", "p_source_item_id" "uuid", "p_snapshot" "jsonb", "p_created_by" "uuid", "p_description" "text", "p_procedure" "text") IS 'Append a Standard TS version and clear prior is_latest in one transaction.';
...
CREATE OR REPLACE FUNCTION "public"."record_invoice_import_atomic"("p_tenant_id" "uuid", "p_user_id" "uuid", "p_operations" "jsonb") RETURNS TABLE("changed_vendor_product_ids" "uuid"[])
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $_$
DECLARE
...
ALTER FUNCTION "public"."record_invoice_import_atomic"("p_tenant_id" "uuid", "p_user_id" "uuid", "p_operations" "jsonb") OWNER TO "postgres";
...
ALTER FUNCTION "public"."save_vendor_items_edit_atomic"("p_tenant_id" "uuid", "p_user_id" "uuid", "p_operations" "jsonb") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."set_invoicing_updated_at"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
  NEW.updated_at = now();
...
ALTER FUNCTION "public"."set_invoicing_updated_at"() OWNER TO "postgres";
...
ALTER TABLE "public"."invoice_lists" OWNER TO "postgres";
...
COMMENT ON COLUMN "public"."invoice_lists"."wholesale_list_id" IS 'Wholesale price list used for invoice line pricing (latest wholesale_price per member).';
...
COMMENT ON COLUMN "public"."items"."delivery" IS 'When true, item appears when Delivery preselect filter is enabled in invoicing template creation.';
...
CREATE TABLE IF NOT EXISTS "public"."menu_cost_list_lines" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "menu_cost_list_id" "uuid" NOT NULL,
    "item_id" "uuid" NOT NULL,
...
ALTER TABLE "public"."menu_cost_list_lines" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."menu_cost_list_members" (
    ...
    "created_by" "uuid" NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "cost_basis" "text" DEFAULT 'corporate'::"text" NOT NULL,
    "price_input_mode" "text" DEFAULT 'price'::"text" NOT NULL,
    CONSTRAINT "menu_cost_list_members_cost_basis_check" CHECK (("cost_basis" = ANY (ARRAY['corporate'::"text", 'wholesale'::"text"]))),
    CONSTRAINT "menu_cost_list_members_price_input_mode_check" CHECK (("price_input_mode" = ANY (ARRAY['price'::"text", 'lcog'::"text"])))
);


ALTER TABLE "public"."menu_cost_list_members" OWNER TO "postgres";


COMMENT ON COLUMN "public"."menu_cost_list_members"."cost_basis" IS 'corporate = scoped breakdown (Costing); wholesale = franchise WL override RPC when linked WL has price.';



COMMENT ON COLUMN "public"."menu_cost_list_members"."price_input_mode" IS 'Pricing Strategy UI: price = enter retail (default); lcog = enter LCOG% and derive retail for display.';
...
ALTER TABLE "public"."menu_cost_lists" OWNER TO "postgres";
...
COMMENT ON COLUMN "public"."standard_technical_sheets"."description" IS 'Standard TS description for this version. v0 is NULL until set via Update.';



COMMENT ON COLUMN "public"."standard_technical_sheets"."procedure" IS 'Standard TS procedure for this version. v0 is NULL until set via Update.';
...
CREATE TABLE IF NOT EXISTS "public"."wholesale_list_lines" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "wholesale_list_id" "uuid" NOT NULL,
    "item_id" "uuid" NOT NULL,
...
ALTER TABLE "public"."wholesale_list_lines" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."wholesale_list_members" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "price_input_mode" "text" DEFAULT 'price'::"text" NOT NULL,
    CONSTRAINT "wholesale_list_members_price_input_mode_check" CHECK (("price_input_mode" = ANY (ARRAY['price'::"text", 'lcog'::"text"])))
);


ALTER TABLE "public"."wholesale_list_members" OWNER TO "postgres";


COMMENT ON COLUMN "public"."wholesale_list_members"."price_input_mode" IS 'Wholesale Costing UI: price = enter wholesale (default); lcog = enter LCOG% and derive wholesale for display.';
...
ALTER TABLE "public"."wholesale_lists" OWNER TO "postgres";
...
ALTER TABLE ONLY "public"."invoice_lists"
    ADD CONSTRAINT "invoice_lists_tenant_name_unique" UNIQUE ("tenant_id", "name");
...
ALTER TABLE ONLY "public"."menu_cost_list_lines"
    ADD CONSTRAINT "menu_cost_list_lines_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."menu_cost_list_members"
    ADD CONSTRAINT "menu_cost_list_members_list_item_unique" UNIQUE ("menu_cost_list_id", "item_id");



ALTER TABLE ONLY "public"."menu_cost_list_members"
    ADD CONSTRAINT "menu_cost_list_members_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."menu_cost_lists"
    ADD CONSTRAINT "menu_cost_lists_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."wholesale_list_lines"
    ADD CONSTRAINT "wholesale_list_lines_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."wholesale_list_members"
    ADD CONSTRAINT "wholesale_list_members_list_item_unique" UNIQUE ("wholesale_list_id", "item_id");



ALTER TABLE ONLY "public"."wholesale_list_members"
    ADD CONSTRAINT "wholesale_list_members_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."wholesale_lists"
    ADD CONSTRAINT "wholesale_lists_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."wholesale_lists"
    ADD CONSTRAINT "wholesale_lists_tenant_name_unique" UNIQUE ("tenant_id", "name");
...
CREATE INDEX "idx_delivery_sites_account_id" ON "public"."delivery_sites" USING "btree" ("account_id");
...
CREATE INDEX "idx_invoice_lists_tenant_id" ON "public"."invoice_lists" USING "btree" ("tenant_id");



CREATE INDEX "idx_invoice_lists_wholesale_list_id" ON "public"."invoice_lists" USING "btree" ("wholesale_list_id");
...
CREATE INDEX "idx_menu_cost_list_lines_list_item_created" ON "public"."menu_cost_list_lines" USING "btree" ("menu_cost_list_id", "item_id", "created_at" DESC);



CREATE INDEX "idx_menu_cost_list_members_list_id" ON "public"."menu_cost_list_members" USING "btree" ("menu_cost_list_id");



CREATE INDEX "idx_menu_cost_lists_tenant_id" ON "public"."menu_cost_lists" USING "btree" ("tenant_id");
...
CREATE INDEX "idx_wholesale_list_lines_list_item_created" ON "public"."wholesale_list_lines" USING "btree" ("wholesale_list_id", "item_id", "created_at" DESC);



CREATE INDEX "idx_wholesale_list_members_item_id" ON "public"."wholesale_list_members" USING "btree" ("item_id");



CREATE INDEX "idx_wholesale_list_members_list_id" ON "public"."wholesale_list_members" USING "btree" ("wholesale_list_id");



CREATE INDEX "idx_wholesale_lists_tenant_id" ON "public"."wholesale_lists" USING "btree" ("tenant_id");
...
CREATE OR REPLACE TRIGGER "delivery_sites_set_updated_at" BEFORE UPDATE ON "public"."delivery_sites" FOR EACH ROW EXECUTE FUNCTION "public"."set_invoicing_updated_at"();



CREATE OR REPLACE TRIGGER "invoice_lists_set_updated_at" BEFORE UPDATE ON "public"."invoice_lists" FOR EACH ROW EXECUTE FUNCTION "public"."set_invoicing_updated_at"();



CREATE OR REPLACE TRIGGER "invoicing_accounts_set_updated_at" BEFORE UPDATE ON "public"."invoicing_accounts" FOR EACH ROW EXECUTE FUNCTION "public"."set_invoicing_updated_at"();
...
ALTER TABLE ONLY "public"."delivery_sites"
    ADD CONSTRAINT "delivery_sites_account_id_fkey" FOREIGN KEY ("account_id") REFERENCES "public"."invoicing_accounts"("id") ON DELETE RESTRICT;
...
ALTER TABLE ONLY "public"."invoice_lists"
    ADD CONSTRAINT "invoice_lists_delivery_site_id_fkey" FOREIGN KEY ("delivery_site_id") REFERENCES "public"."delivery_sites"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "public"."invoice_lists"
    ADD CONSTRAINT "invoice_lists_tenant_id_fkey" FOREIGN KEY ("tenant_id") REFERENCES "public"."tenants"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."invoice_lists"
    ADD CONSTRAINT "invoice_lists_wholesale_list_id_fkey" FOREIGN KEY ("wholesale_list_id") REFERENCES "public"."wholesale_lists"("id") ON DELETE RESTRICT;
...
ALTER TABLE ONLY "public"."menu_cost_list_lines"
    ADD CONSTRAINT "menu_cost_list_lines_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "public"."users"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "public"."menu_cost_list_lines"
    ADD CONSTRAINT "menu_cost_list_lines_item_id_fkey" FOREIGN KEY ("item_id") REFERENCES "public"."items"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."menu_cost_list_lines"
    ADD CONSTRAINT "menu_cost_list_lines_menu_cost_list_id_fkey" FOREIGN KEY ("menu_cost_list_id") REFERENCES "public"."menu_cost_lists"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."menu_cost_list_members"
    ADD CONSTRAINT "menu_cost_list_members_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "public"."users"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "public"."menu_cost_list_members"
    ADD CONSTRAINT "menu_cost_list_members_item_id_fkey" FOREIGN KEY ("item_id") REFERENCES "public"."items"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."menu_cost_list_members"
    ADD CONSTRAINT "menu_cost_list_members_menu_cost_list_id_fkey" FOREIGN KEY ("menu_cost_list_id") REFERENCES "public"."menu_cost_lists"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."menu_cost_lists"
    ADD CONSTRAINT "menu_cost_lists_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "public"."users"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "public"."menu_cost_lists"
    ADD CONSTRAINT "menu_cost_lists_tenant_id_fkey" FOREIGN KEY ("tenant_id") REFERENCES "public"."tenants"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."wholesale_list_lines"
    ADD CONSTRAINT "wholesale_list_lines_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "public"."users"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "public"."wholesale_list_lines"
    ADD CONSTRAINT "wholesale_list_lines_item_id_fkey" FOREIGN KEY ("item_id") REFERENCES "public"."items"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."wholesale_list_lines"
    ADD CONSTRAINT "wholesale_list_lines_wholesale_list_id_fkey" FOREIGN KEY ("wholesale_list_id") REFERENCES "public"."wholesale_lists"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."wholesale_list_members"
    ADD CONSTRAINT "wholesale_list_members_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "public"."users"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "public"."wholesale_list_members"
    ADD CONSTRAINT "wholesale_list_members_item_id_fkey" FOREIGN KEY ("item_id") REFERENCES "public"."items"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."wholesale_list_members"
    ADD CONSTRAINT "wholesale_list_members_wholesale_list_id_fkey" FOREIGN KEY ("wholesale_list_id") REFERENCES "public"."wholesale_lists"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."wholesale_lists"
    ADD CONSTRAINT "wholesale_lists_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "public"."users"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "public"."wholesale_lists"
    ADD CONSTRAINT "wholesale_lists_tenant_id_fkey" FOREIGN KEY ("tenant_id") REFERENCES "public"."tenants"("id") ON DELETE CASCADE;
...
CREATE POLICY "invoice_lists_tenant_access" ON "public"."invoice_lists" USING (("tenant_id" IN ( SELECT "p"."tenant_id"
   FROM "public"."profiles" "p"
  WHERE ("p"."user_id" = "auth"."uid"())))) WITH CHECK (("tenant_id" IN ( SELECT "p"."tenant_id"
   FROM "public"."profiles" "p"
  WHERE ("p"."user_id" = "auth"."uid"()))));
...
CREATE POLICY "menu_cost_list_lines_tenant_access" ON "public"."menu_cost_list_lines" USING (("menu_cost_list_id" IN ( SELECT "mcl"."id"
   FROM ("public"."menu_cost_lists" "mcl"
     JOIN "public"."profiles" "p" ON (("p"."tenant_id" = "mcl"."tenant_id")))
  WHERE ("p"."user_id" = "auth"."uid"())))) WITH CHECK (("menu_cost_list_id" IN ( SELECT "mcl"."id"
   FROM ("public"."menu_cost_lists" "mcl"
     JOIN "public"."profiles" "p" ON (("p"."tenant_id" = "mcl"."tenant_id")))
  WHERE ("p"."user_id" = "auth"."uid"()))));
...
CREATE POLICY "menu_cost_list_members_tenant_access" ON "public"."menu_cost_list_members" USING (("menu_cost_list_id" IN ( SELECT "mcl"."id"
   FROM ("public"."menu_cost_lists" "mcl"
     JOIN "public"."profiles" "p" ON (("p"."tenant_id" = "mcl"."tenant_id")))
  WHERE ("p"."user_id" = "auth"."uid"())))) WITH CHECK (("menu_cost_list_id" IN ( SELECT "mcl"."id"
   FROM ("public"."menu_cost_lists" "mcl"
...
CREATE POLICY "menu_cost_lists_tenant_access" ON "public"."menu_cost_lists" USING (("tenant_id" IN ( SELECT "p"."tenant_id"
   FROM "public"."profiles" "p"
  WHERE ("p"."user_id" = "auth"."uid"())))) WITH CHECK (("tenant_id" IN ( SELECT "p"."tenant_id"
   FROM "public"."profiles" "p"
  WHERE ("p"."user_id" = "auth"."uid"()))));
...
CREATE POLICY "wholesale_list_lines_tenant_access" ON "public"."wholesale_list_lines" USING (("wholesale_list_id" IN ( SELECT "wl"."id"
   FROM ("public"."wholesale_lists" "wl"
     JOIN "public"."profiles" "p" ON (("p"."tenant_id" = "wl"."tenant_id")))
  WHERE ("p"."user_id" = "auth"."uid"())))) WITH CHECK (("wholesale_list_id" IN ( SELECT "wl"."id"
   FROM ("public"."wholesale_lists" "wl"
     JOIN "public"."profiles" "p" ON (("p"."tenant_id" = "wl"."tenant_id")))
  WHERE ("p"."user_id" = "auth"."uid"()))));
...
CREATE POLICY "wholesale_list_members_tenant_access" ON "public"."wholesale_list_members" USING (("wholesale_list_id" IN ( SELECT "wl"."id"
   FROM ("public"."wholesale_lists" "wl"
     JOIN "public"."profiles" "p" ON (("p"."tenant_id" = "wl"."tenant_id")))
  WHERE ("p"."user_id" = "auth"."uid"())))) WITH CHECK (("wholesale_list_id" IN ( SELECT "wl"."id"
   FROM ("public"."wholesale_lists" "wl"
...
CREATE POLICY "wholesale_lists_tenant_access" ON "public"."wholesale_lists" USING (("tenant_id" IN ( SELECT "p"."tenant_id"
   FROM "public"."profiles" "p"
  WHERE ("p"."user_id" = "auth"."uid"())))) WITH CHECK (("tenant_id" IN ( SELECT "p"."tenant_id"
   FROM "public"."profiles" "p"
  WHERE ("p"."user_id" = "auth"."uid"()))));
...
GRANT ALL ON FUNCTION "public"."backfill_labor_into_standard_snapshot"("p_snapshot" "jsonb", "p_source_item_id" "uuid", "p_tenant_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."backfill_labor_into_standard_snapshot"("p_snapshot" "jsonb", "p_source_item_id" "uuid", "p_tenant_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."backfill_labor_into_standard_snapshot"("p_snapshot" "jsonb", "p_source_item_id" "uuid", "p_tenant_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."calculate_item_costs_with_breakdown_wholesale_overrides"("p_tenant_id" "uuid", "p_call_depth" integer, "p_seed_item_ids" "uuid"[], "p_wholesale_list_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."calculate_item_costs_with_breakdown_wholesale_overrides"("p_tenant_id" "uuid", "p_call_depth" integer, "p_seed_item_ids" "uuid"[], "p_wholesale_list_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."calculate_item_costs_with_breakdown_wholesale_overrides"("p_tenant_id" "uuid", "p_call_depth" integer, "p_seed_item_ids" "uuid"[], "p_wholesale_list_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."ensure_standard_technical_sheet_v0_if_absent"("p_tenant_id" "uuid", "p_source_item_id" "uuid", "p_snapshot" "jsonb", "p_created_by" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."ensure_standard_technical_sheet_v0_if_absent"("p_tenant_id" "uuid", "p_source_item_id" "uuid", "p_snapshot" "jsonb", "p_created_by" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."ensure_standard_technical_sheet_v0_if_absent"("p_tenant_id" "uuid", "p_source_item_id" "uuid", "p_snapshot" "jsonb", "p_created_by" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."insert_standard_technical_sheet_version_atomic"("p_tenant_id" "uuid", "p_source_item_id" "uuid", "p_snapshot" "jsonb", "p_created_by" "uuid", "p_description" "text", "p_procedure" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."insert_standard_technical_sheet_version_atomic"("p_tenant_id" "uuid", "p_source_item_id" "uuid", "p_snapshot" "jsonb", "p_created_by" "uuid", "p_description" "text", "p_procedure" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."insert_standard_technical_sheet_version_atomic"("p_tenant_id" "uuid", "p_source_item_id" "uuid", "p_snapshot" "jsonb", "p_created_by" "uuid", "p_description" "text", "p_procedure" "text") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."record_invoice_import_atomic"("p_tenant_id" "uuid", "p_user_id" "uuid", "p_operations" "jsonb") TO "anon";
GRANT ALL ON FUNCTION "public"."record_invoice_import_atomic"("p_tenant_id" "uuid", "p_user_id" "uuid", "p_operations" "jsonb") TO "authenticated";
GRANT ALL ON FUNCTION "public"."record_invoice_import_atomic"("p_tenant_id" "uuid", "p_user_id" "uuid", "p_operations" "jsonb") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."save_vendor_items_edit_atomic"("p_tenant_id" "uuid", "p_user_id" "uuid", "p_operations" "jsonb") TO "anon";
GRANT ALL ON FUNCTION "public"."save_vendor_items_edit_atomic"("p_tenant_id" "uuid", "p_user_id" "uuid", "p_operations" "jsonb") TO "authenticated";
GRANT ALL ON FUNCTION "public"."save_vendor_items_edit_atomic"("p_tenant_id" "uuid", "p_user_id" "uuid", "p_operations" "jsonb") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."set_invoicing_updated_at"() TO "anon";
GRANT ALL ON FUNCTION "public"."set_invoicing_updated_at"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."set_invoicing_updated_at"() TO "service_role";
...
GRANT ALL ON TABLE "public"."delivery_sites" TO "anon";
GRANT ALL ON TABLE "public"."delivery_sites" TO "authenticated";
GRANT ALL ON TABLE "public"."delivery_sites" TO "service_role";
...
GRANT ALL ON TABLE "public"."invoicing_accounts" TO "anon";
GRANT ALL ON TABLE "public"."invoicing_accounts" TO "authenticated";
GRANT ALL ON TABLE "public"."invoicing_accounts" TO "service_role";
...
GRANT ALL ON TABLE "public"."menu_cost_list_lines" TO "anon";
GRANT ALL ON TABLE "public"."menu_cost_list_lines" TO "authenticated";
GRANT ALL ON TABLE "public"."menu_cost_list_lines" TO "service_role";



GRANT ALL ON TABLE "public"."menu_cost_list_members" TO "anon";
GRANT ALL ON TABLE "public"."menu_cost_list_members" TO "authenticated";
GRANT ALL ON TABLE "public"."menu_cost_list_members" TO "service_role";
...
GRANT ALL ON TABLE "public"."wholesale_list_lines" TO "anon";
GRANT ALL ON TABLE "public"."wholesale_list_lines" TO "authenticated";
GRANT ALL ON TABLE "public"."wholesale_list_lines" TO "service_role";



GRANT ALL ON TABLE "public"."wholesale_list_members" TO "anon";
GRANT ALL ON TABLE "public"."wholesale_list_members" TO "authenticated";
GRANT ALL ON TABLE "public"."wholesale_list_members" TO "service_role";



GRANT ALL ON TABLE "public"."wholesale_lists" TO "anon";
GRANT ALL ON TABLE "public"."wholesale_lists" TO "authenticated";
GRANT ALL ON TABLE "public"."wholesale_lists" TO "service_role";
