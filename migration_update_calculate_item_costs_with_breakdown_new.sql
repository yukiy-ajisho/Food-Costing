-- =========================================================
-- Migration: Update calculate_item_costs_with_breakdown function
-- to accept p_user_id parameter for multi-tenancy
-- FIXED: Added support for 'ml' and 'gallon' units in Prepped Items calculation
-- =========================================================
-- This migration updates the calculate_item_costs_with_breakdown
-- function to filter by user_id for data isolation, while preserving
-- all original logic including specific_child handling, unit conversions,
-- and labor cost inheritance.
-- 
-- FIX: Added 'ml' and 'gallon' cases to Prepped Items calculation
-- to fix issue where non-mass units were being ignored.
-- =========================================================

-- 既存の関数を削除
DROP FUNCTION IF EXISTS calculate_item_costs_with_breakdown(uuid);

-- 新しい関数を作成（p_user_idパラメータ付き、元のロジックを保持、ml/gallon対応追加）
CREATE OR REPLACE FUNCTION calculate_item_costs_with_breakdown(p_user_id uuid)
RETURNS TABLE (
  out_item_id uuid,
  out_item_name text,
...
BEGIN
  ...
  -- ステップ1: Raw Itemsのコストを計算（user_idでフィルタリング、specific_child処理を保持）
  ...
  WHERE i.item_kind = 'raw'
    ...
    AND (rl.user_id = p_user_id OR rl.user_id IS NULL)
    AND (bi.user_id = p_user_id OR bi.user_id IS NULL)
  GROUP BY i.id, i.name, i.each_grams, i.base_item_id, bi.specific_weight, rl.specific_child
  ON CONFLICT (item_id) DO NOTHING;

  -- ステップ2: ループでPrepped Itemsを計算（user_idでフィルタリング、labor cost継承ロジックを保持）
  LOOP
    ...
    WHERE parent.item_kind = 'prepped'
      ...
      AND rl.user_id = p_user_id
      AND child_items.user_id = p_user_id
      AND (bi.user_id = p_user_id OR bi.user_id IS NULL)
  ...
  -- ステップ3: Labor コストを追加して最終結果を返す（user_idでフィルタリング）
  RETURN QUERY
  WITH labor_costs AS (
    SELECT
      rl.parent_item_id as item_id,
...
-- 完了
-- =========================================================
-- このマイグレーションにより、mlとgallon単位の材料も
-- Prepped Itemsのコスト計算に正しく含まれるようになります
