-- =========================================================
-- テスト用: コスト計算ロジック（SELECTで結果を返す）
-- =========================================================
-- このSQLはテスト用で、結果をSELECTで返します
-- 動作確認後、関数として実装します
-- =========================================================
-- 修正方針:
-- - ステップ1（Raw Itemsの事前計算）を削除
-- - ステップ2で、各recipe_lineごとにspecific_childを考慮してRaw Itemのコストを計算
-- - Express側の実装と一致させる
-- =========================================================

-- テスト用パラメータ（実際のテスト時は変更してください）
-- p_user_id: テストするユーザーID
-- p_item_ids: 計算するアイテムIDの配列（NULLの場合は全アイテム）

-- 一時テーブル: Item単位でコストを管理（セッションレベルで作成）
...
DO $$
DECLARE
  -- テスト用: 実際のuser_idに置き換えてください
  p_user_id uuid := 'YOUR_USER_ID_HERE'::uuid;
  -- テスト用: NULLの場合は全アイテム、特定のアイテムのみ計算する場合は配列を指定
  -- 例: ARRAY['item_id_1'::uuid, 'item_id_2'::uuid]
...
SELECT 
  ...
  proceed_yield_unit,
  each_grams
FROM temp_item_costs
WHERE cost_per_gram IS NOT NULL
ORDER BY item_name;

-- 一時テーブルをクリーンアップ（セッション終了時に自動的に削除されます）
-- DROP TABLE IF EXISTS temp_item_costs;
